...
function drawScatterChart() {
  ...
  if (lib === 'plotly') {
    const xVals = x.map(v => parseFloat(v)).filter(v => !isNaN(v));
    const yVals = yData[0].values.filter(v => !isNaN(v));  // assume first Y col for bounds
    ...
    const renderChart = (xMin, xMax, yMin, yMax) => {
      const filtered = filteredData.filter(row => {
        ...
        return !isNaN(xv) && xv >= xMin && xv <= xMax;
      });

      const xFiltered = filtered.map(row => parseFloat(row[xCol]));

      const filteredYData = yCols.map(col => {
        return {
          name: col,
          values: filtered.map(row => parseFloat(row[col]))
        };
      });

      const traces = filteredYData.map((yd, i) => {
        const yFiltered = yd.values.map(v => (v >= yMin && v <= yMax ? v : null));
        return {
          x: xFiltered,
          y: yFiltered,
          ...
          marker: {
            ...
            size: subtype === 'Bubble' ? yFiltered.map(v => Math.abs(v) / 2 || 5) : 8
          }
        };
      });
      ...
      Plotly.react('scatterChartContainer', traces, layout);
    };

    // Initial render
    renderChart(minX, maxX, minY, maxY);

    // Range inputs and listeners
...
function drawLineChart() {
  ...
  if (lib === 'plotly') {
    ...
    const sliderDiv = document.getElementById('lineFilterControls') || document.createElement('div');
    sliderDiv.id = 'lineFilterControls';
    sliderDiv.className = 'mb-3';
    document.getElementById('lineTab').insertBefore(sliderDiv, document.getElementById('lineChartContainer'));

    sliderDiv.innerHTML = `
      <label class="form-label text-light">X-Axis Range: <span id="lineXRangeLabel">${minX} - ${maxX}</span></label>
      <input type="range" id="lineXMin" min="${minX}" max="${maxX}" value="${minX}" step="1" class="form-range mb-2">
      <input type="range" id="lineXMax" min="${minX}" max="${maxX}" value="${maxX}" step="1" class="form-range">
    `;

    const renderChart = (xMin, xMax) => {
      const filtered = filteredData.filter(row => {
        const xv = parseFloat(row[xCol]);
        return !isNaN(xv) && xv >= xMin && xv <= xMax;
      });
      ...
      const xFiltered = filtered.map(row => parseFloat(row[xCol]));
      const yFiltered = yCols.map(col => ({
        name: col,
        values: filtered.map(row => parseFloat(row[col]))
      }));

      const traces = yFiltered.map((yd, i) => ({
        x: xFiltered,
        y: yd.values,
        type: 'scatter',
        mode: 'lines+markers',
      ...
      Plotly.react('lineChartContainer', traces, layout);
    };

    renderChart(minX, maxX);

    const get = id => parseFloat(document.getElementById(id).value);
    const update = () => {
      let xMin = get('lineXMin'), xMax = get('lineXMax');
      if (xMin > xMax) [xMin, xMax] = [xMax, xMin];

      document.getElementById('lineXRangeLabel').textContent = `${xMin} - ${xMax}`;
      renderChart(xMin, xMax);
    };

    ['lineXMin', 'lineXMax'].forEach(id =>
